...
Wireless capture files contains a lot of frames that we are not interested. Airdecap-ng allows us to remove the Wireless headers from unencrypted capture files. If we have the passphrase for WPA we can also supply it and decrypt the traffic. We can also use the option *-b* to filter on a particular BSSID. 

```
sudo airdecap-ng -b 34:08:04:09:3D:38 opennet-01.cap
```

If we have the passphrase, we can decrypt WPA traffic and remove the wireless frames headers, like so:

```
sudo airdecap-ng -p 'passphrase' -e 'ESSID' -b 'BSSID' test-01.cap
```

![[Pasted image 20241219104922.png]]
...
The Airodump-ng CSV files contain the relationship between wireless clients and APs, and the list of probed networks. They can be passed to Airgraph-ng to create two types of graphs, a Clients to AP relationship and a Clients to Probe graph. 

#### Clients to AP Relationship Graph

The Clients to AP Relationship (CAPR) graph type display the relationship between clients and APs. As its focus is more on the clients rather than APs, it won't draw an AP that doesn't have a client. Airgraph-ng assigns colors to the access points depending on the type of encryption provided. green is for WPA, yellow is for WEP, red is for Open, and black is for unknown.
...
First we need to download the below file and store it under */usr/share/airgraph-ng*
...
Below is a sample graph generated by Airgraph-ng:

![[Pasted image 20241219120523.png]]

#### Clients Probe Graph

The Client Probe Graph (CPG) displays the relationship between the clients and the networks probed. We can use the option *-g* CPG.
...
![[Pasted image 20241219121729.png]]
